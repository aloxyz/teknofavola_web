<script lang="ts">
	...
	let { lang, email }: { lang: Locale; email: string } = $props();
</script>
...
<div role="alert" style="padding:clamp(28px,5vw,72px) clamp(24px,5vw,80px) 70px;display:flex;flex-direction:column;gap:20px;align-items:flex-start">
	...
	<div style="display:flex;flex-wrap:wrap;gap:2px">
		<button type="button" onclick={() => location.reload()} class="tf-btn-accent" style="display:flex;align-items:center;gap:10px;background:var(--tf-accent);color:var(--tf-bg);border:0;padding:14px 18px;font-weight:800;font-size:13px;cursor:pointer;text-align:left">{t(lang, 'retry')}</button>
